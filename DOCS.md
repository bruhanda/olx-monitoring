...
| `NotifyTimes` | `DAILY_NOTIFY_TIMES` | `11:00,15:00,20:00` | час доби `HH:MM`; лише **початкове** значення — далі розклад живе в БД і редагується на `/settings` |
...
| `CountOrphanListings()` / `DeleteOrphanListings()` | записи без прив'язки до пошуку: порахувати / прибрати |
| `GetSearchByID(id)` | один пошук; `ErrSearchNotFound`, якщо немає |
| `GetSetting(key)` / `SetSetting(key, value)` | налаштування, що редагуються з адмінки (таблиця `settings`) |
...
`Run(stop)` на кожній ітерації **заново читає розклад** через провайдер
`WithTimes`, тому зміни з адмінки застосовуються без рестарту:
- якщо список порожній — режим інтервалу `PollInterval`, перший запуск одразу
  (на цьому тримається процедура прогріву бази з DEPLOY.md);
- інакше — очікування до `nextOccurrence()`, далі `runOnce()`; час наступного
...
- `Reload()` будить планувальник негайно, щоб перерахувати розклад. Виклик
  неблокуючий: зайві сигнали відкидаються.

Розклад має **хвилинну гранулярність**: час, що припадає на поточну хвилину,
вважається минулим і переноситься на завтра.
...
Підсумок запуску (`runSummary`) — одне повідомлення. Однакові помилки
групуються: якщо впали всі пошуки (типово при блокуванні за IP), у звіті буде
один рядок замість переліку, плюс підказка про ймовірну причину (403 / 429 /
таймаут / DNS).
...
| `/clear-orphans` | POST | видалити оголошення, не прив'язані до жодного пошуку |
| `/settings` | GET/POST | перегляд і зміна розкладу перевірок; застосовується без рестарту |
...
- **Міграція `search_id`.** Записи, створені до появи поля, мають `search_id = 0`.
  ...
  буде; кнопка очищення для них спрацює лише після такого бекфілу. Ті, що так і
  не трапились повторно (оголошення зійшло з видачі), лишаються «осиротілими»:
  їх не видно на сторінках пошуків, і прибрати їх можна кнопкою
  «🧹 Видалити N записів» на головній.

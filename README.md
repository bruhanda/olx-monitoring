...
- 📜 **Історія по кожному пошуку** — усе колись знайдене з датою першої появи,
  окремою сторінкою на кожне посилання.
- ⏰ **Розклад під себе** — кілька разів на добу в заданий час, редагується
  на сторінці налаштувань і застосовується без перезапуску.
...
| `DAILY_NOTIFY_TIMES` | `11:00,15:00,20:00` | початковий розклад; далі редагується на `/settings` і зберігається в БД |
...
`/` — список пошуків із лічильником зібраних оголошень, форма додавання і
поточний розклад. `/listings?id=N` — історія знайденого по конкретному пошуку:
назва, ціна, локація, дата першої появи, посилання на OLX, по 50 на сторінку.
`/settings` — розклад перевірок: змінюється в один клік, планувальник
підхоплює його одразу. Операції: `/add`, `/edit`, `/delete`, `/activate`,
`/deactivate`, `/clear` (забути зібрані оголошення пошуку, щоб отримати їх
як нові).
